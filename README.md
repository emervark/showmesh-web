...
- `public/sitemap.xml` — written by hand. Add a `<url>` entry when a page is
  added. It is not generated: `@astrojs/sitemap` pulls optional native packages
  whose Linux entries npm cannot write into `package-lock.json` from Windows, so
  `npm ci` then fails on Cloudflare's builders. Keep the marketing site free of
  npm `dependencies` unless a lockfile is generated on Linux or in CI.
